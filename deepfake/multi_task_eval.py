...
def eval():
    ...
    model = init_model(cfg, mode='eval', type=cfg.model_type, path=model_path)
    model.to(device)
    # transform setting
    ...
    eval_dataloader = build_dataloader(eval_dataset, shuffle=False, batch_size=cfg.batch_size,num_workers=cfg.num_workers)
    # eval
    ...
    with torch.no_grad():
        for data in tqdm(eval_dataloader):
            ...
            if type(output)==tuple:
                seg, stu_cla = output[0],output[1]
            predict = stu_cla.data.cpu().numpy().argsort()[:,-1:].squeeze().tolist()
            predict_all.extend(predict)
            label_all.extend(label.cpu().numpy().tolist())
            # save the predict seg mask
            # see_seg_index = predict.index(2)
            # print(np.sum(seg[see_seg_index].data.cpu().numpy()>0.5),predict_all,label_all)
            # mask_check = seg[see_seg_index].data.cpu().numpy()    
            # mask_check = np.squeeze(mask_check, 0)       #H*W
            # mask_check = 255*(mask_check>0.5)
            # cv2.imwrite('check.jpg', mask_check)
            # break
...
CUDA_VISIBLE_DEVICES=1 python multi_task_eval.py configs/f3net_seg.py --out out.json
'''
