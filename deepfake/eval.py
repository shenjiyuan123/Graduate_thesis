...
from deepfake.train import init_model
...
def eval():
    ...
    # load model
    model_path = os.path.join(cfg.model_save_path,'latest_model.pth')
    model = init_model(mode='eval', path=model_path)
    model.to(device)
    # transform setting
    ...
    sns.heatmap(matrix/np.sum(matrix), annot=True, fmt='.2%', cmap="YlGnBu", linewidths=.5, xticklabels=[0,1,2], yticklabels=[0,1,2],ax=ax)
