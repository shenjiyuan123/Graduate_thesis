...
class F3Net(nn.Module):
    def __init__(self, num_classes=1, img_width=224, img_height=224, LFS_window_size=10, LFS_stride=2, LFS_M = 6, mode='FAD', pretrain=None):
        ...
        # init branches
        if mode == 'FAD' or mode == 'Both' or mode == 'Mix':
    ...
    def init_xcep_FAD(self):
        ...
        # Xception original is 32,3,3,3, but we need 32,12,3,3.
        # If need to load pretrain, we need to copy on conv1
        if self.pretrain is not None:
            # To get a good performance, using ImageNet-pretrained Xception model is recommended
            state_dict = get_xcep_state_dict(pretrained_path=self.pretrain)
        ...
        # if don't load pretrain, that means load self-trained, change the original xception conv1 module
        else:
            self.FAD_xcep.conv1 = nn.Conv2d(12, 32, 3, 2, 0, bias=False)
...
def get_xcep_state_dict(pretrained_path='/home/shenjiyuan/deepfake/deepfake/checkpoint/F3Net/xception-b5690688.pth'):
    # load Xception
